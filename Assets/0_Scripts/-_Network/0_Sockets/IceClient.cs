...
namespace ICENet
{
    public class IceClient : MonoBehaviour, IClientHandler
    {
        ...
        [HideInInspector] public bool ReleaseOnDestroy = true;
        ...
        public void Connect(string address, ushort port, IntPtr udp)
        {
            _socket = new _iceClientSocket(GetComponent<IceClient>());
            _socket.Bind(udp);
            _socket.Connect(address, port);
        ...
        {
            _socket?.Disconnect(release);
            _socket = null;
        }

        public void Disconnect(bool release) => Release(release);

        private void OnDestroy() => Release(ReleaseOnDestroy);
    }
}